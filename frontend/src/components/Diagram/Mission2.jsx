...
const Mission2 = ({ missionId, onComplete }) => {
  ...
  const [spinResultQuestion, setSpinResultQuestion] = useState(null);
  ...
  const spinwheelOptionsMap = useMemo(() => ({
    ...
    'menjodohkan': 'Menjodohkan Diagram',
    'ya-tidak': 'Ya atau Tidak',
    'gambar-isian': 'Gambar + Isian',
    'open-ended': 'Soal Terbuka'
  }), []);
  ...
  }, [questionsByTopic, answeredQuestions]);
    ...
    const fetchQuestions = async () => {
      ...
      try {
        ...
        if (response.ok && Array.isArray(data) && data.length > 0) {
          setAllQuestions(data.map((q, index) => ({ ...q, id: index + 1 }))); // Assign IDs if not present
          const organized = {};
          data.forEach((q, index) => {
            const qWithId = { ...q, id: index + 1 };
            if (!organized[q.type]) organized[q.type] = [];
            organized[q.type].push(qWithId);
          });
          ...
          setError('Gagal memuat soal atau tidak ada soal untuk misi ini');
        }
      } catch (err) {
  ...
  const retryFetch = () => {
    ...
    const fetchQuestions = async () => {
      try {
        ...
        const data = await response.json();
        if (response.ok && Array.isArray(data) && data.length > 0) {
          setAllQuestions(data.map((q, index) => ({ ...q, id: index + 1 })));
          const organized = {};
          data.forEach((q, index) => {
            const qWithId = { ...q, id: index + 1 };
            if (!organized[q.type]) organized[q.type] = [];
  ...
  const handleSpin = useCallback(() => {
    ...
    const numSegments = availableQuestions.length;
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        setSpinResultQuestion(selectedQuestion);
  ...
  const handleSubmitAnswer = async (answer) => {
    ...
    switch (currentQuestion.type) {
      ...
      case 'open-ended': {
        ...
        isCorrect = matchedCount >= Math.ceil(correctKeywords.length * 0.8) || currentQuestion.type === 'open-ended';
        break;
      }
      case 'gambar-isian':
        isCorrect = answer.trim().toLowerCase() === currentQuestion.correct_answer.trim().toLowerCase();
        break;
      case 'menjodohkan': {
        const sortedUserAnswers = Object.keys(userMatchingAnswers).sort().reduce((obj, key) => {
          obj[key] = userMatchingAnswers[key];
          return obj;
        }, {});
  ...
  const renderSpinwheel = () => {
    ...
    if (numSegments === 0 && allQuestions.length === 0) {
      return null; // Handled by isLoading or error
    }
      ...
      <div className="space-y-12">
        ...
        <div className="bg-gradient-to-br from-slate-900/95 to-stone-900/95 backdrop-blur-sm rounded-3xl p-10 border-2 border-amber-600/30 shadow-2xl">
          <div className="text-center mb-10">
            ...
            <p className="text-amber-200/80 text-lg">Putar roda untuk memilih soal kristal ilmu!</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center">
            <div className="relative w-full max-w-[500px] aspect-square">
              ...
              >
                {availableQuestions.map((question, index) => {
                  ...
                  const displayText = `${question.question_text.substring(0, 15)}${question.question_text.length > 15 ? '...' : ''}`;
            ...
            {spinResultQuestion && !spinning && (
              <div className="mt-8 text-center">
                <div className="text-3xl font-bold text-amber-200 animate-pulse">
                  Soal Terpilih: {spinResultQuestion.question_text.substring(0, 30)}{spinResultQuestion.question_text.length > 30 ? '...' : ''}!
                </div>
                <button 
                  onClick={() => setSpinResultQuestion(null)} 
      ...
      <div className="bg-gradient-to-br from-slate-900/95 to-stone-900/95 backdrop-blur-sm rounded-3xl p-8 border-2 border-amber-500/30 shadow-2xl w-full max-w-3xl mx-auto relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-6">
            ...
            {(currentQuestion.type === 'audio-isian' || currentQuestion.type === 'gambar-isian') && (
            ...
            {currentQuestion.type === 'menjodohkan' && (
              <div className="text-center mt-8">
                <button
                  ...
                  disabled={showFeedback || Object.keys(userMatchingAnswers).length !== currentQuestion.options.length}
                  className="px-8 py-4 bg-gradient-to-r from-amber-600 to-yellow-600 text-white font-bold rounded-xl shadow-lg hover:from-amber-700 hover:to-yellow-700 disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-105 transition-all duration-300 text-lg border-2 border-amber-500/20"
                  aria-label="Kirim jawaban"
                >
                  <span className="flex items-center justify-center gap-2">
            ...
            {showFeedback && (
              <div className={`mt-8 text-center transform animate-bounce ${result ? 'animate-pulse' : ''}`}>
                <div className={`inline-block p-6 rounded-2xl border-2 ${
                  ...
                  {result && currentQuestion.type === 'menjodohkan' && (
                    <div className="text-green-200 mt-2 text-lg flex flex-col items-center">
                      <p>"Jawaban Benar:"</p>
                      {Object.entries(currentQuestion.correct_answer).map(([item, url]) => (
                        <div key={item} className="flex items-center gap-2">
                          <span>{item}</span>
                          <img src={url} alt="Jawaban Benar" className="w-16 h-16 object-contain rounded" />
                        </div>
                      ))}
                    </div>
                  )}
                  {result && ['pg', 'benar-salah', 'ya-tidak', 'audio-isian', 'gambar-isian'].includes(currentQuestion.type) && (
